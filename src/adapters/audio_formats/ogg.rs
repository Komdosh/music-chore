...
mod tests {
    ...
    fn test_extract_metadata_from_tags_maps_all_supported_fields() {
        ...
        let tagged_file = make_tagged_file(
            245.5,
            vec![
                TagItem::new(ItemKey::TrackTitle, ItemValue::Text("Track Name".to_string())),
                TagItem::new(ItemKey::TrackArtist, ItemValue::Text("Artist Name".to_string())),
                TagItem::new(ItemKey::AlbumTitle, ItemValue::Text("Album Name".to_string())),
                TagItem::new(
                    ItemKey::AlbumArtist,
        ...
        assert_eq!(metadata.title.as_ref().map(|v| v.value.as_str()), Some("Track Name"));
        assert_eq!(
        ...
        assert_eq!(metadata.genre.as_ref().map(|v| v.value.as_str()), Some("Trip-Hop"));
    ...
    fn test_extract_metadata_from_tags_uses_recording_date_and_folder_fallbacks() {
        ...
        let tagged_file = make_tagged_file(
            60.25,
            vec![
                TagItem::new(ItemKey::TrackTitle, ItemValue::Text("Only Title".to_string())),
        ...
        assert_eq!(metadata.title.as_ref().map(|v| v.value.as_str()), Some("Only Title"));
        assert_eq!(
            metadata.artist.as_ref().map(|v| v.value.as_str()),
