...
fn test_library_deserialization() {
  ...
  "total_files": 1
    ...
    assert_eq!(
        library.artists[0].albums[0].tracks[0]
            .metadata
            .title
            .as_ref()
            .unwrap()
...
fn test_library_serialization_deserialization_roundtrip() {
    ...
    // Create a test track
    let track = create_test_track(
        Some("Round Trip Track"),
        Some("Round Trip Artist"),
        Some("Round Trip Album"),
        "round/trip/path/track.flac",
    );
    ...
    let album_node = AlbumNode {
        ...
        year: Some(2024),
        tracks: vec![track_node],
        files: vec![PathBuf::from("round/trip/path/track.flac")]
            .into_iter()
            .collect(),
    ...
    assert_eq!(
        original_library.total_artists,
        deserialized_library.total_artists
    );
    assert_eq!(
        original_library.total_albums,
        deserialized_library.total_albums
    );
    assert_eq!(
        original_library.total_tracks,
        deserialized_library.total_tracks
    );
    assert_eq!(
        original_library.total_files,
        deserialized_library.total_files
    );

    // Check specific content
    assert_eq!(
        original_library.artists[0].name,
        deserialized_library.artists[0].name
    );
    assert_eq!(
        original_library.artists[0].albums[0].title,
        deserialized_library.artists[0].albums[0].title
    );
    assert_eq!(
        original_library.artists[0].albums[0].year,
        deserialized_library.artists[0].albums[0].year
    );
    assert_eq!(
        original_library.artists[0].albums[0].tracks[0]
            .metadata
            .title
            .as_ref()
            .unwrap()
            .value,
        deserialized_library.artists[0].albums[0].tracks[0]
            .metadata
            .title
            .as_ref()
            .unwrap()
...
fn test_library_multiple_albums_same_artist_serialization() {
    ...
    let track1 = create_test_track(
        ...
        Some("Same Artist"),
        Some("Album 1"),
        "same_artist/album1/track1.flac",
    );
    let track_node1 = TrackNode {
        file_path: track1.file_path.clone(),
        metadata: track1.metadata.clone(),
    ...
    let album_node1 = AlbumNode {
        ...
        year: Some(2023),
        tracks: vec![track_node1],
        files: vec![PathBuf::from("same_artist/album1/track1.flac")]
            .into_iter()
            .collect(),
    ...
    let track2 = create_test_track(
        ...
        Some("Same Artist"),
        Some("Album 2"),
        "same_artist/album2/track2.flac",
    );
    let track_node2 = TrackNode {
        file_path: track2.file_path.clone(),
        metadata: track2.metadata.clone(),
    ...
    let album_node2 = AlbumNode {
        ...
        year: Some(2024),
        tracks: vec![track_node2],
        files: vec![PathBuf::from("same_artist/album2/track2.flac")]
            .into_iter()
            .collect(),
    ...
    let album_titles: Vec<&String> = deserialized.artists[0]
        .albums
        .iter()
        .map(|a| &a.title)
        .collect();
...
fn test_library_multiple_tracks_same_album_serialization() {
    ...
    let track1 = create_test_track(
        Some("Track 1"),
        Some("Multi Track Artist"),
        Some("Multi Track Album"),
        "multi_artist/multi_album/track1.flac",
    );
    let track_node1 = TrackNode {
        file_path: track1.file_path.clone(),
        metadata: track1.metadata.clone(),
    ...
    let track2 = create_test_track(
        ...
        Some("Multi Track Artist"),
        Some("Multi Track Album"),
        "multi_artist/multi_album/track2.flac",
    );
    let track_node2 = TrackNode {
        file_path: track2.file_path.clone(),
        metadata: track2.metadata.clone(),
    ...
    let track_titles: Vec<&String> = deserialized.artists[0].albums[0]
        .tracks
        .iter()
...
fn test_library_metadata_source_serialization() {
    ...
    let album_node = AlbumNode {
        ...
        year: Some(2023),
        tracks: vec![track_node],
        files: vec![PathBuf::from("test/path/track.flac")]
            .into_iter()
            .collect(),
    ...
    assert_eq!(
        track_metadata.title.as_ref().unwrap().source,
        MetadataSource::Embedded
    );
    assert_eq!(
        track_metadata.artist.as_ref().unwrap().source,
        MetadataSource::FolderInferred
    );
    assert_eq!(
        track_metadata.album.as_ref().unwrap().source,
        MetadataSource::UserEdited
    );
    assert_eq!(
        track_metadata.album_artist.as_ref().unwrap().source,
...
fn test_library_metadata_confidence_serialization() {
    ...
    let track_metadata = TrackMetadata {
        ...
        album: Some(MetadataValue::inferred(
            "Medium Confidence Album".to_string(),
            0.7,
        )),
        album_artist: None,
        track_number: Some(MetadataValue::embedded(1)),
    ...
    let album_node = AlbumNode {
        ...
        year: Some(2023),
        tracks: vec![track_node],
        files: vec![PathBuf::from("test/confidence/track.flac")]
            .into_iter()
            .collect(),
...
fn test_library_with_checksum_serialization() {
    ...
    let album_node = AlbumNode {
        ...
        year: Some(2023),
        tracks: vec![track_node],
        files: vec![PathBuf::from("test/checksum/track.flac")]
            .into_iter()
            .collect(),
    ...
    let deserialized: Library = serde_json::from_str(&json_str).unwrap();
    assert_eq!(
        deserialized.artists[0].albums[0].tracks[0].file_path,
        PathBuf::from("test/checksum/track.flac")
    );
}
