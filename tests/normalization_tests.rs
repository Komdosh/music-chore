...
fn test_to_title_case_with_punctuation() {
    assert_eq!(to_title_case("hello, world!"), "Hello, World!");
    assert_eq!(
        to_title_case("it's a wonderful life"),
        "It's A Wonderful Life"
    );
    assert_eq!(
        to_title_case("the lord of the rings: fellowship"),
...
fn test_to_title_case_with_contractions() {
    assert_eq!(
        to_title_case("can't help falling in love"),
        "Can't Help Falling In Love"
    );
    assert_eq!(to_title_case("i'm yours"), "I'm Yours");
    assert_eq!(
        to_title_case("we're all in this together"),
...
fn test_normalize_genre_aliases() {
    ...
    assert_eq!(
        normalize_genre("alternative"),
        Some("Alternative".to_string())
    ); // Alternative is a standard genre, not an alias for Rock
}
...
fn test_normalize_genre_slash_separated() {
    ...
    assert_eq!(
        normalize_genre("electronic/dance"),
        Some("Electronic/Dance".to_string())
    );
    assert_eq!(
        normalize_genre("hip-hop/rap"),
        Some("Hip-Hop/Hip-Hop".to_string())
    ); // Both hip-hop and rap map to Hip-Hop
    assert_eq!(normalize_genre("rock / pop"), Some("Rock/Pop".to_string()));
}
...
fn test_normalize_genre_unknown() {
    assert_eq!(
        ...
        Some("Unknown Genre".to_string())
    ); // Unknown genres get title-cased
    assert_eq!(
        normalize_genre("made up genre"),
        Some("Made Up Genre".to_string())
    ); // Unknown genres get title-cased
...
fn test_normalize_genre_electronic_aliases() {
    ...
    assert_eq!(
        normalize_genre("drum and bass"),
        Some("Drum And Bass".to_string())
    ); // Drum and bass is not an alias for Electronic
...
fn test_normalize_genre_rock_aliases() {
    assert_eq!(normalize_genre("rock"), Some("Rock".to_string()));
    assert_eq!(
        normalize_genre("classic rock"),
        Some("Classic Rock".to_string())
    ); // Classic rock is not an alias for Rock
    assert_eq!(normalize_genre("hard rock"), Some("Hard Rock".to_string())); // Hard rock is not an alias for Rock
    assert_eq!(
        normalize_genre("progressive rock"),
        Some("Progressive Rock".to_string())
    ); // Progressive rock is not an alias for Rock
}
...
fn test_normalize_and_format_with_real_files() {
    ...
    std::fs::copy(
        ...
        album_dir.join("lowercase title.flac"),
    )
    .unwrap();
