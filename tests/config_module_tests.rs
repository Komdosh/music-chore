...
fn test_app_config_new_creates_default() {
    ...
    assert_eq!(config.max_recursion_depth, default_config.max_recursion_depth);
    assert_eq!(config.follow_symlinks, default_config.follow_symlinks);
    assert_eq!(config.default_inference_confidence, default_config.default_inference_confidence);
    ...
    assert_eq!(config.skip_metadata_by_default, default_config.skip_metadata_by_default);
    assert_eq!(config.excluded_extensions, default_config.excluded_extensions);
    assert_eq!(config.excluded_directories, default_config.excluded_directories);
    assert_eq!(config.show_source_indicators, default_config.show_source_indicators);
...
fn test_app_config_builder_fluent_interface() {
    ...
    assert_eq!(config.excluded_extensions, vec!["tmp".to_string(), "log".to_string()]);
    assert_eq!(config.excluded_directories, vec![".git".to_string(), "temp".to_string()]);
...
fn test_app_config_validation_invalid_recursion_depth() {
    ...
    assert!(result.unwrap_err().to_string().contains("Maximum recursion depth should not exceed"));
}
...
fn test_app_config_validation_invalid_confidence_low() {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Default inference confidence must be between"));
}
...
fn test_app_config_validation_zero_file_size() {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Maximum file size must be greater than"));
}
...
fn test_app_config_save_and_load() {
    ...
    assert_eq!(original_config.max_recursion_depth, loaded_config.max_recursion_depth);
    assert_eq!(original_config.follow_symlinks, loaded_config.follow_symlinks);
    assert_eq!(original_config.default_inference_confidence, loaded_config.default_inference_confidence);
    assert_eq!(original_config.max_file_size_mb, loaded_config.max_file_size_mb);
    assert_eq!(original_config.verbose_by_default, loaded_config.verbose_by_default);
    assert_eq!(original_config.skip_metadata_by_default, loaded_config.skip_metadata_by_default);
    assert_eq!(original_config.excluded_extensions, loaded_config.excluded_extensions);
    assert_eq!(original_config.excluded_directories, loaded_config.excluded_directories);
    assert_eq!(original_config.show_source_indicators, loaded_config.show_source_indicators);
    assert_eq!(original_config.process_cue_files, loaded_config.process_cue_files);
}
