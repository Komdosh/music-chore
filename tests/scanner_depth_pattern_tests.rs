...
fn test_scan_dir_with_depth_deep_nesting() {
    ...
    for i in 1..=6 {
        ...
        if i < 6 {
            // Don't create another dir after the last file
...
fn test_scan_dir_with_options_exclude_multiple_patterns() {
    ...
    assert_eq!(
        tracks.len(),
        2,
        "Expected 2 files after excluding *.tmp, backup*, and *.bak patterns"
    );
    let filenames: Vec<String> = tracks
        .iter()
        .map(|t| {
...
fn test_scan_dir_with_options_exclude_directory_pattern() {
    ...
    assert_eq!(
        tracks.len(),
        2,
        "Expected 2 files after excluding temp/** pattern"
    );
    let filenames: Vec<String> = tracks
        .iter()
        .map(|t| {
...
fn test_scan_dir_with_options_deterministic_ordering() {
    ...
    let filenames: Vec<String> = tracks1
        .iter()
        .map(|t| {
            t.file_path
                .file_name()
